...
- 索引的数据结构和底层实现
  ...
  - B+Tree：[数据结构整理-B+Tree](/2019/01/01/data-structure/#B+Tree)
...
共享锁和排他锁也叫读锁（read lock）和写锁（write lock），由这两种锁组成的锁系统可以处理并发读或者并发写问题。

读锁是共享的，相互不阻塞；写锁是排他的，会阻塞其他的写锁和读锁，以确保在给定的时间里只有一个用户能执行写入，并防止其他用户读取正在写入的同一资源。
...
- 意向锁定义
  ...
  - intention shared lock (IS) 表示某事务意图获取表中个别 row 的 S 锁
    - `SELECT ... LOCK IN SHARE MODE` 加 IS 锁
  - intention exclusive lock (IX) 表示事务意图获取表中个别 row 的 X 锁
